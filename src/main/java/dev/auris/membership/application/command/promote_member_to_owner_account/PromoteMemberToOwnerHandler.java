...
@Service
public class PromoteMemberToOwnerHandler {
    private final LoadMembershipPort loadMembershipPort;
    private final SaveMembershipPort saveMembershipPort;

    public PromoteMemberToOwnerHandler(LoadMembershipPort loadMembershipPort, SaveMembershipPort saveMembershipPort) {
        this.loadMembershipPort = loadMembershipPort;
        this.saveMembershipPort = saveMembershipPort;
    }

    @Transactional
    public PromoteMemberToOwnerResponse handle(PromoteMemberToOwnerCommand command) {
        ...
        target.promoteToOwner(actor);
        saveMembershipPort.save(target);
        return PromoteMemberToOwnerResponse.from(target);
    }
